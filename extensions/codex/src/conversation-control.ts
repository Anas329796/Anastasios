...
export async function setCodexConversationModel(params: {
  ...
  const binding = await requireThreadBinding(params.sessionFile, params.isolationKey);
  const runtime = resolveCodexAppServerRuntimeOptions({ pluginConfig: params.pluginConfig });
...
async function requireThreadBinding(sessionFile: string, isolationKey?: string) {
  const binding = await readCodexAppServerBinding(sessionFile, { isolationKey });
  if (!binding?.threadId) {
