...
describe("createCopilotNativeWebSearchWrapper", () => {
  ...
  it("activates for eligible model with auto provider", () => {
    ...
    const trackingStreamFn = (model: any, context: any, options: any) => {
      // Check if payload was patched (tools array modified)
      if (options?.body) {
        const body = JSON.parse(options.body);
        if (body.tools?.some((t: any) => t.type === "web_search")) payloadPatched = true;
      }
      return { __mock: true };
    };
    ...
    const result = wrapper(model as any, {} as any, {} as any);
    // streamWithPayloadPatch returns the underlying streamFn result
    // The key is that it didn't bail early (skip conditions)
