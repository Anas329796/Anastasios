...
function isInheritedCodexConfigTable(headerLine: string): boolean {
...
function extractInheritedCodexRuntimeConfig(configToml: string): string {
  ...
  for (const rawLine of configToml.split(/\r?\n/)) {
    ...
    if (trimmedLine.startsWith("[")) {
      inInheritedTable = isInheritedCodexConfigTable(trimmedLine);
      if (inInheritedTable) {
        if (sawInheritedContent && inheritedLines[inheritedLines.length - 1] !== "") {
          inheritedLines.push("");
        }
        inheritedLines.push(rawLine.trimEnd());
        sawInheritedContent = true;
      }
      continue;
    }
    ...
    const assignment = /^(?<key>[A-Za-z0-9_-]+)\s*=\s*(?<value>.+)$/.exec(semanticLine);
    if (!assignment?.groups) {
      continue;
    }
    if (!INHERITED_TOP_LEVEL_CODEX_CONFIG_KEYS.has(assignment.groups.key)) {
      continue;
    }
    inheritedLines.push(rawLine.trimEnd());
    sawInheritedContent = true;
