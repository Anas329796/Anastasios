...
describe("sendPolicy deny — suppress delivery, not processing (#53328)", () => {
  ...
  it("does not route plugin-owned bindings under message-tool-only when send policy denies delivery", async () => {
    setNoAbort();
    hookMocks.runner.hasHooks.mockImplementation(
      ((hookName?: string) =>
        hookName === "inbound_claim" || hookName === "message_received") as () => boolean,
    ...
    sessionBindingMocks.resolveByConversation.mockReturnValue({
      bindingId: "binding-message-tool-deny",
      targetSessionKey: "plugin-binding:codex:abc123",
      targetKind: "session",
      conversation: {
        channel: "telegram",
    ...
    expect(sessionBindingMocks.touch).toHaveBeenCalledWith("binding-message-tool-deny");
    expect(hookMocks.runner.runInboundClaimForPluginOutcome).not.toHaveBeenCalled();
    expect(replyResolver).toHaveBeenCalledTimes(1);
    expect(dispatcher.sendFinalReply).not.toHaveBeenCalled();
  });
  ...
  it("routes plugin-owned bindings under message-tool-only source delivery", async () => {
    ...
    expect(hookMocks.runner.runInboundClaimForPluginOutcome).toHaveBeenCalledTimes(1);
    expect(replyResolver).not.toHaveBeenCalled();
    expect(dispatcher.sendFinalReply).not.toHaveBeenCalled();
  });

  it("suppresses plugin-bound fallback notices under message-tool-only source delivery", async () => {
    setNoAbort();
    hookMocks.runner.hasHooks.mockImplementation(
      ((hookName?: string) =>
        hookName === "inbound_claim" || hookName === "message_received") as () => boolean,
