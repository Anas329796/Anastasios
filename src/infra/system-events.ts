...
function findDuplicateInQueue(
  ...
  if (contextKey === null) {
    ...
    return last &&
      isDuplicateSystemEvent(last, { text, contextKey, deliveryContext, trusted, audience })
      ? last
      : undefined;
  }
  for (const event of queue) {
    if (isDuplicateSystemEvent(event, { text, contextKey, deliveryContext, trusted, audience })) {
      return event;
    }
  }
  return undefined;
...
function isDuplicateSystemEvent(
  ...
  incoming: Pick<SystemEvent, "text" | "contextKey" | "deliveryContext" | "trusted" | "audience">,
): boolean {
  return (
