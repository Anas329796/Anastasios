...
function joinNonEmptyTextFromOutboundPayloads(
...
function resolveCronAwarenessText(params: {
  ...
  const outboundText = params.outboundPayloads?.length
    ? joinNonEmptyTextFromOutboundPayloads(params.outboundPayloads)
    : undefined;
  if (outboundText) {
    return outboundText;
  }
