...
describe("session MCP runtime", () => {
  ...
  it("resolves runtime scope from cfg, defaulting to session", () => {
    expect(__testing.resolveSessionMcpRuntimeScope(undefined)).toBe("session");
    expect(__testing.resolveSessionMcpRuntimeScope({})).toBe("session");
    expect(__testing.resolveSessionMcpRuntimeScope({ mcp: { servers: {} } })).toBe("session");
    expect(
      __testing.resolveSessionMcpRuntimeScope({ mcp: { servers: {}, runtimeScope: "session" } }),
    ).toBe("session");
    expect(
      __testing.resolveSessionMcpRuntimeScope({ mcp: { servers: {}, runtimeScope: "shared" } }),
    ).toBe("shared");
  });
