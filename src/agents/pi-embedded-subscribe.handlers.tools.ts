...
import { resolveStrictToolMode, shouldAllowToolArgumentAlias } from "./tool-strictness.js";
  ...
  const continueToolExecutionStart = () => {
    ...
    if (toolName === "read") {
      ...
      const strictToolMode = resolveStrictToolMode({
        env: process.env,
        strict: ctx.params.strictToolMode ?? ctx.params.config?.strictToolMode,
      });
      ...
      if (!hasCanonicalPath && hasAliasPath && !shouldAllowToolArgumentAlias(strictToolMode)) {
