...
describe("acquireSessionWriteLock", () => {
  ...
  it("reclaims payload-less orphan lock files once they outlast the init grace", async () => {
    await withTempSessionLockFile(async ({ sessionFile, lockPath }) => {
      await fs.writeFile(lockPath, "", "utf8");
      // Older than ORPHAN_LOCK_PAYLOAD_GRACE_MS (30 s) so reclaim is allowed.
  ...
  it("does not reclaim a payload-less orphan lock within the init grace window", async () => {
    await withTempSessionLockFile(async ({ sessionFile, lockPath }) => {
      await fs.writeFile(lockPath, "", "utf8");
      // Within ORPHAN_LOCK_PAYLOAD_GRACE_MS (30 s): a writer that paused for
      // CPU pressure or container freeze must not have its lock stolen yet.
      const orphanDate = new Date(Date.now() - 5_000);
      await fs.utimes(lockPath, orphanDate, orphanDate);
