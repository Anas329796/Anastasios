...
// and the owner metadata write. The grace must outlast realistic writer stalls
// (CPU saturation, container freeze, long GC pauses) so we never steal a lock
// from a writer that simply hasn't flushed its payload yet; the effective
// reclaim age is still bounded by the caller's `staleMs` via `Math.min`, so
// 10 s callers continue to recover at or before their staleness budget.
