...
settings and governed workspace declarations. The base model is intentionally
small:

- `policy.jsonc` stores operator-owned requirements.
- Existing OpenClaw settings and workspace declarations are observed as
  evidence; policy does not create a second configuration system.
...
This version applies that model to channel conformance, tool metadata
conformance, MCP server conformance, model-provider conformance, and network
SSRF posture:

- `policy.jsonc` stores channel, tool metadata, MCP server, model-provider,
