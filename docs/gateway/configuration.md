...
<AccordionGroup>
  ...
  <Accordion title="Set up webhooks (hooks)">
    ...
    - Hook agent runs are isolated by default. Use `sessionMode: "persistent"` only for trusted integrations with stable, scoped `sessionKey` values that intentionally share multi-turn context.
