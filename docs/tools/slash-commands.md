...
- `/codex status|models|threads|resume|compact|review|diagnostics|account|mcp|skills|goal` inspects and controls the bundled Codex app-server harness. See [Codex harness](/plugins/codex-harness).
- `/goal status|set|pause|resume|clear` inspects and controls the bound Codex thread goal.
- QQBot-only commands:
  - `/bot-ping`
  - `/bot-version`
  - `/bot-help`
