...
It also emits `security.trust_model.multi_user_heuristic` when config suggests likely shared-user ingress (for example open DM/group policy, configured group targets, or wildcard sender rules) and that ingress can reach runtime/filesystem contexts that are not guarded by the effective sandbox/tool policy.
For intentional shared-user setups, the audit guidance is to sandbox the shared ingress path (including `sandbox.mode="non-main"` for group/channel sessions), keep filesystem access workspace-scoped, and keep personal/private identities or credentials off that runtime.
