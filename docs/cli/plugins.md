...
- Non-bundled conversation hooks (`llm_input`, `llm_output`, `before_model_resolve`, `before_agent_reply`, `before_agent_run`, `before_agent_finalize`, `agent_end`, `model_failure_terminal`) require `plugins.entries.<id>.hooks.allowConversationAccess=true`.
